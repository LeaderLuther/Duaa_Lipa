...
def user_details(id):
   if request.method=='POST' :
      ...
      with engine.connect() as conn:
        ...
        conn.commit()
      return redirect(url_for('photos',id=id))
   if x==id :
    print(x)
    print("user_details")
...
#       with engine.connect() as conn:
#         description = conn.execute(text("UPDATE user_table SET User_description=:photo1 WHERE User_ID=:id"), {'photo1': photo1, 'photo2': photo2, 'photo3': photo3, description:'description'})
#         photo1 = conn.execute(text("UPDATE user_table SET Photolink1=:photo1 WHERE User_ID=:id"), {'photo1': photo1, 'photo2': photo2, 'photo3': photo3, description:'description'})
#         photo2 = conn.execute(text("UPDATE user_table SET Photolink2=:photo2 WHERE User_ID=:id"), {'photo1': photo1, 'photo2': photo2, 'photo3': photo3, description:'description'})
#         photo3 = conn.execute(text("UPDATE user_table SET Photolink3=:photo3 WHERE User_ID=:id"), {'photo1': photo1, 'photo2': photo2, 'photo3': photo3, description:'description'})
#         #conn.commit()
#       return redirect(url_for('show_for_you',user_id=id))
#    if x==id :
#     print(x)
#     print("picutes")
#     return render_template('PicutesAryan.html',id=id)
#    else :
#     print(x)
#     print("failed")
#     return "Error page not found"
...
def photos(id):
    if request.method == 'POST':
        ...
        with engine.connect() as conn:
            ...
            conn.commit()
        return redirect(url_for('show_for_you', user_id=id))
    if request.args.get('action') == 'get_photos':
        with engine.connect() as conn:
            result = conn.execute(text("SELECT Photolink1, Photolink2, Photolink3 FROM user_table WHERE User_ID=:id"), {'id': id})
            row = result.fetchone()
        photo_links = [url_for('serve_photo', filename=row[i]) if row[i] else None for i in range(3)]
