...
def calculate_time_difference(datetime1, datetime2):
    ...
    # Return the time difference components
...
def load_likes_from_db(user_id):
  with engine.connect() as conn:
    ...
    for row in rows:
      likes.append(row._asdict())
      curr_dt = likes[-1]['Like_datetime']
      days, hours, minutes, seconds = calculate_time_difference(curr_dt, datetime.now())
      likes[-1]['Days'] = days
      likes[-1]['Hours'] = hours
    return likes
...
def load_matches_from_db(user_id):
  with engine.connect() as conn:
    ...
    curr_dt = matches[-1]['Matched_datetime']
    days, hours, minutes, seconds = calculate_time_difference(curr_dt, datetime.now())
    matches[-1]['Days'] = days
    matches[-1]['Hours'] = hours
    return matches
...
def load_messages_from_db(sender_id, receiver_id):
  with engine.connect() as conn:
    ...
    for row in rows:
      ...
      curr_dt = messages[-1]['Text_datetime']
      days, hours, minutes, seconds = calculate_time_difference(curr_dt, datetime.now())
      messages[-1]['Days'] = days
      messages[-1]['Hours'] = hours
    return messages
